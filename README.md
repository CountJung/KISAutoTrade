...
[![Release](https://img.shields.io/github/v/release/CountJung/KISAutoTrade?include_prereleases)](https://github.com/CountJung/KISAutoTrade/releases)
...
16. [프로덕션 빌드](#프로덕션-빌드)
17. [웹 접속](#웹-접속)
18. [라이선스 및 면책 조항](#라이선스-및-면책-조항)

---

## 개요

KISAutoTrade는 **한국투자증권(KIS) Open API**를 사용하는 개인용 자동 주식 매매 시스템입니다.
...
│       ├── storage/        # JSON 파일 I/O (거래, 통계, 쟑고)
│       ├── notifications/  # Discord 봇 알림
│       ├── server/         # axum 웹 서버 (포트 7474, 웹 접속용)
...
```bash
git clone https://github.com/CountJung/KISAutoTrade.git
cd KISAutoTrade
```
...
설정 파일은 **종류마다 저장 위치가 다릅니다**.

#### ① 프로젝트 루트 (클론한 폴더)

Git에 추적되지 않는 민감 파일이며, 없으면 앱이 자동 생성하거나 수동으로 만듭니다.

| 파일 | 내용 |
|------|------|
| `secure_config.json` | Discord 봇 토큰·채널 ID (수동 생성 — `secure_config.example.json` 참고) |
| `.env` | 웹 서버 포트 등 환경변수 (앱 최초 실행 시 자동 생성, 없으면 기본값 사용) |

#### ② OS 앱 데이터 경로 (Tauri `app_data_dir`)

**`profiles.json`** 과 거래·통계 데이터는 Tauri 표준 앱 데이터 디렉토리에 저장됩니다.  
프로젝트 루트가 **아닌** 아래 경로를 확인하세요.

| OS | 앱 데이터 경로 |
|----|---------------|
| **macOS** | `~/Library/Application Support/com.countjung.kisautotrade/` |
| **Windows** | `%APPDATA%\com.countjung.kisautotrade\` |
| **Linux** | `~/.local/share/com.countjung.kisautotrade/` |

```
com.countjung.kisautotrade/
├── profiles.json       ← KIS 앱 키, 계좌번호 (Settings UI에서 자동 생성)
└── data/
    ├── trades/         ← 체결 내역 (YYYY/MM/DD/trades.json)
    ├── orders/         ← 주문 기록
    ├── stats/          ← 일별 통계
    └── balance/        ← 잔고 스냅샷
```

> ⚠️ `profiles.json`에는 APP SECRET이 **평문**으로 저장됩니다.  
> macOS의 `~/Library/Application Support/`는 기본적으로 iCloud Drive 동기화 대상이 아니지만,  
> 타사 백업 소프트웨어가 해당 경로를 포함하는 경우가 있으니 주의하세요.

#### ③ 로그 파일

로그는 앱 실행 위치(CWD) 기준 `logs/` 하위에 저장됩니다.

```
{프로젝트 루트}/logs/
├── app.log.YYYY-MM-DD      ← INFO 이상 전체 로그 (KST 시간대)
└── error.log.YYYY-MM-DD    ← WARN/ERROR 로그 (KST 시간대)
```
...
### 프론트엔드만 빌드 (React 앱 서비스용)

```bash
npm run build:web
# dist/ 폴더 생성 → KISAutoTrade 실행 시 웹 서버로 서비스됨
```
...
| Trading | 종목 현재가 조회 (돋보기 버튼+로딩), 매수/매도 주문 폼, 체결 내역 |
| Strategy | 이동평균 전략 파라미터 설정, 전략 ON/OFF |
| History | 기간별 거래 기록 및 일별 통계 테이블 |
| Log | 레벨 필터 + 키워드 검색 로그 븷어 |
| Settings | **계좌 프로파일 관리**, 테마, **웹 접속 포트 설정**, Discord 테스트, 진단 |
...
> **설정 위치**: Strategy 화면 → 각 전략 카드에서 종목코드·수량·MA기간을 직접 편집 후 저장

---

### OrderManager — 주문 실행 흐름

자동 매매 신호는 다음 순서로 처리됩니다.

```
전략 신호
  │
  ▼
① RiskManager.can_trade() 검증
   ├─ 비상 정지 상태? → 주문 차단
   └─ 일일 손실 한도 초과? → 주문 차단
  │
  ▼
② RiskManager.check_position_size() 검증
   └─ 총 잔고 대비 종목 비중 초과? → 주문 차단
  │
  ▼
③ 동일 종목 미체결 주문 중복 확인
   └─ 이미 대기 중? → 주문 건너뜀
  │
  ▼
④ KIS API place_order() 호출 (rate-limit 시 1초 재시도 × 3회)
  │
  ▼
⑤ 미체결 주문 풀(HashMap) 등록
  │
  ▼
⑥ 체결 이벤트 수신 (WebSocket H0STCNI0 / 폴링)
  │
  ▼
⑦ PositionTracker 연동 → StatsStore / RiskManager 손익 반영
  │
  ▼
⑧ Discord 체결 알림 전송
```
...
**Strategy 화면 → 리스크 관리 섹션**에서 UI로 설정·저장할 수 있습니다.

| 항목 | 기본값 | 설명 |
|------|--------|------|
| **일일 손실 한도** | 500,000원 | 하루 최대 허용 손실 금액. 초과 시 비상 정지 발동 |
| **종목당 최대 비중** | 20% | 총 잔고 대비 단일 종목에 투자 가능한 최대 비율 |

> 💡 **팁**: 처음에는 손실 한도를 낮게(예: 10만원) 설정하고 모의투자로 충분히 테스트하세요.

#### 비상 정지 (Emergency Stop)

일일 손실 한도를 초과하면 자동으로 비상 정지가 발동되어 **추가 주문이 모두 차단**됩니다.  
비상 정지는 **자동으로 해제되지 않습니다.** Strategy 화면 상단의 빨간 배너에서 직접 해제 버튼을 클릭해야 합니다.

#### 미체결 주문 현황

Strategy 화면 하단에서 현재 KIS API에 접수된 미체결 주문 목록(종목·구분·수량·주문번호·신호이유)을 **5초 간격**으로 실시간 확인할 수 있습니다.
...
| OS | 경로 |
|----|------|
| Windows | `%APPDATA%\KISAutoTrade\` |
| macOS | `~/Library/Application Support/KISAutoTrade/` |
...
| OS | 경로 |
|----|------|
| Windows | `%APPDATA%\KISAutoTrade\log\` |
| macOS | `~/Library/Application Support/KISAutoTrade/log/` |
...
KISAutoTrade는 데스크탑 앱 실행 시 **포트 7474에 웹 서버가 자동 시작**됩니다.
스마트폰/태블릿 등 같은 네트워크의 다른 기기에서 브라우저로 접속할 수 있습니다.

### 접속 방법

1. KISAutoTrade 데스크탑 앱을 실행합니다.
2. 모바일 기기에서 브라우저 접속:
   ```
   http://데스크탑-IP-주소:7474
   ```
   - Windows: `ipconfig` 에서 IPv4 주소 확인
   - macOS: `ifconfig en0` 에서 주소 확인

### 전체 UI 접속 (React SPA)

`dist/` 폴더가 있어야 React 앱 UI가 서비스됩니다.
없으면 모바일 대시보드 HTML이 표시됩니다.

```bash
# 프론트엔드 빌드 후 dist/ 폴더를 바이너리 옷에 놓으면 자동 서비스됩니다.
npm run build:web
# Windows: dist/ 폴더를 .exe 옛 폴더에 복사
# macOS:   dist/ 폴더를 앱 번듮l 내부에 포함 (빌드 시 자동 삽입)
```
...
| URL | 설명 |
|-----|------|
| `GET /api/info` | 앱 코드 / 버전 / 포트 |
| `GET /api/balance` | 잔고 JSON |
| `GET /api/price/:symbol` | 종목코드(6자리) 현재가 |
| `GET /api/executed` | 당일 체결 내역 |

### 포트 변경

**Settings > 웹 접속 설정** 에서 포트를 변경하고 저장하면 `.env` 파일에 `WEB_PORT=새포트`가 기록됩니다.
씦1 재시작 후 새 포트가 적용됩니다.

또는 시작 전 환경변수로 직접 지정:
```bash
WEB_PORT=8080 ./KISAutoTrade
```
