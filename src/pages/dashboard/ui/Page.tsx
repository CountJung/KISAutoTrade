...
export default function Dashboard() {
  ...
  const tossTotal = brokerHoldings.reduce(
    (acc, item) => {
      const quantity = parseDecimalString(item.quantity)
      const marketValue = parseDecimalString(item.currentPrice.amount) * quantity
      const pnl = parseDecimalString(item.unrealizedPnl?.amount)
                  ...
                  {brokerHoldings.map((item) => {
                    ...
                    return (
                      <TableRow key={`${item.brokerId}:${item.market}:${item.symbol}`} hover>
                        ...
                        <TableCell align="right">{fmtDecimalString(item.quantity, 6)}</TableCell>
                        <TableCell align="right" sx={{ display: { xs: 'none', sm: 'table-cell' } }}>
