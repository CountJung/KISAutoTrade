...
- [ ] 리스크 설정과 일별 runtime 상태를 재시작 후 복원한다.
  - 설정값과 일별 손익·비상정지·주문 횟수·연속 손실 차단 상태를 분리 저장한다.
  - runtime 상태는 broker가 확인한 체결/주문 기록으로 재구축하고 날짜·broker/account scope별로 격리한다.
  - 앱 재시작으로 손실 한도·쿨다운·연속 손실 차단을 우회할 수 없게 한다.
